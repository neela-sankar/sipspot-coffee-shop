    ...
    <section className='h-[60vh] xl:h-[70vh]'>
        <div className='container mx-auto h-full flex items-center'>
            <Swiper navigation={true} modules={[Navigation]} className='h-[400px]'>
                {testimonails.map((test, index) => {
                    return (
                        <SwiperSlide key={index} className='w-full h-full'>
                            <div className='flex justify-center h-full xl:pt-14'>
                                <div className='max-w-[60%] text-secondary'>
                                    <IoMdQuote className='text-6xl text-primary mb-12 mx-auto' />
                                    <p className='text-2xl text-center mb-8'>{test.message}</p>
                                    <div className='text-center'>
                                        <p className='text-xl font-bold mb-1 text-primary'>{test.name}</p>
                                        <p className='text-third'>{test.profession}</p>
                                    </div>
                                </div>
                            </div>
                        </SwiperSlide>
